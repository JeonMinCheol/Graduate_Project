...
class Client:
    ...
    def _printClients(self):
        for client in self.clients:
            ...
            print(f"client_id:{idx} | iter:{round} | model:{self.args.model_name} | batch:{batch} | optimizer:{self.args.optim} | train delay:{delay} | transfer delay:{transfer}")
    ...
    def _createClient(self):
        ...
        for idx in range(self.n_clients):
            ...
            if self.args.data == "MNIST" or self.args.data == "FashionMNIST" or self.args.data == "EMNIST":
                ...
                setting.extend([model, optim.SGD(model.parameters(), lr=self.lr, momentum=self.momentum), nn.CrossEntropyLoss(ignore_index=-1), self.host, idx, 0, idx % self.gpu if self.args.use_multiple_gpu else 1])            
